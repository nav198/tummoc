//
//  PoojaDailyNeedsTableViewCell.swift
//  tummoc
//
...
class PoojaDailyNeedsTableViewCell: UITableViewCell, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        let PoojaDailyNeeds = shoppingCartData.filter({$0.name == "Pooja Daily Needs"}).first?.items?.count ?? 0
        return PoojaDailyNeeds
    }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        guard let itemsValue = shoppingCartData.filter({ $0.name == "Pooja Daily Needs" }).first?.items else { return cell }
    ...
    func addingToCartAction(at indexPath: IndexPath) {
        guard var category = shoppingCartData.first(where: { $0.name == "Pooja Daily Needs" }),
