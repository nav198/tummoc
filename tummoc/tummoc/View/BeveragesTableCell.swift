...
class BeveragesTableCell: UITableViewCell, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
...
//    var monuDelegate:MonuSelectionViewDelegate?
//    var bookmarkDelegate:BookmarkDelegate?
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        let BeveragesCount = shoppingCartData.filter({$0.name == "Beverages"}).first?.items?.count ?? 0
        return BeveragesCount
    }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        guard let itemsValue = shoppingCartData.filter({ $0.name == "Beverages" }).first?.items else { return cell }
    ...
    func addingToCartAction(at indexPath: IndexPath) {
        guard var category = shoppingCartData.first(where: { $0.name == "Beverages" }),
    ...
    func addToFavouritesAction(at indexPath: IndexPath) {
        ...
        do {
            ...
            shoppingCartData.removeAll(where: { $0.name == "Beverages" })
